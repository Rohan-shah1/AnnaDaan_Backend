...
exports.updateReservationStatus = async (req, res) => {
  try {
    ...
    if (!isDonor && !isReceiver) {
      console.error(`User ${userId} not authorized for reservation ${id}`);
  ...
  try {
    ...
    if (req.user.userType !== 'admin') {
      if (donationId) {
        ...
        // If no donationId, just return my reservations
    ...
    const reservations = await Reservation.find(query)
      ...
      .populate({
        ...
        populate: {
          path: 'donor',
          select: 'name organizationName phone city avatar'
        }
      });
...
exports.submitRating = async (req, res) => {
  try {
    ...
    const isDonor = reservation.donation.donor._id.toString() === userId.toString();
